            ...
            let harvestContainers = creep.room.find(FIND_STRUCTURES, {
                filter : (structure) => structure.structureType === STRUCTURE_CONTAINER
                                            ...
                                            && (!Memory.structures['id'+structure.id].harvester
                                                || (Game.getObjectById(Memory.structures['id'+structure.id].harvester).ticksToLive < (Game.getObjectById(Memory.structures['id'+structure.id].harvester).memory.ticksBeforeWork-5)))
            });
