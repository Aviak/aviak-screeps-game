...
var roleCourierLvl3 = {
                ...
                target = creep.pos.findClosestByPath(FIND_STRUCTURES, {
                    filter: (structure) => {
                        return (structure.structureType === STRUCTURE_EXTENSION ||
                            ...
                            (structure.structureType === STRUCTURE_TOWER && (structure.energyCapacity-structure.energy) >= 100)) && structure.energyCapacity > structure.energy;
                    }
                });
            }
            if(!target) {
    ...
    processOnDeathEffect : function(creepName) {
        if(Memory.creeps[creepName].target) {
            console.log(Memory.creeps[creepName].requested);
